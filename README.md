...
В данном проекте создана база данных со знаниями о клиентах розничных сетей, а также написано представление и процедуры, необходимые для создания персональных предложений.

## Logical view of database model

Данные, описанные в таблицах из раздела [Входные данные](#входные-данные) подгружаются с csv либо tsv файлов (процедуры import/export)
...
Cкрипт *part1.sql*, создает базу данных и таблицы, описанные выше в разделе [входные данные](#входные-данные).

В скрипте присутствуют процедуры, позволяющие импортировать и экспортировать данные для каждой таблицы из файлов/в файлы с расширением *.csv* и *.tsv*. \
В качестве параметра каждой процедуры для импорта из *csv* файла указывается разделитель.

Некоторые тестовые данные находятся в папке *datasets*.

## Part 2. Создание представлений

В скрипте *part2.sql*, создаются представления, описанные выше в разделе [выходные данные](#выходные-данные).

*Более подробная информация по логике получения каждого поля содержится в папке materials.*

## Part 3. Ролевая модель

В скрипте *part3.sql* создаются роли и осуществляется выдача им прав в соответствии с описанным ниже.
...
В скрипте *part4.sql*, содержится функция, определяющая предложения, ориентированные на рост среднего чека
...
В скрипте *part5.sql* содержится функция, определяющая предложения, ориентированные на рост частоты визитов
...
В скрипте *part6.sql*, содержится функция, определяющая предложения, ориентированные на кросс-продажи (рост маржи)
